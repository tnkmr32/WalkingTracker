...
1. **屋外歩行状態での再測定（最優先）**
   今回の検証はほぼ静止状態（ログ上の緯度経度がほとんど変化していない）で実施したことが判明している。GNSS/FusedLocationProviderClientには、端末の静止を検知すると省電力のため取得間隔を内部的に延長する既知の挙動があり、要求interval（5000ms）とは無関係に約7.5秒へ延びている可能性がある。[gps-poc-plan.md](gps-poc-plan.md)の検証手順が求める屋外・歩行状態（画面ON/OFF・バックグラウンドを含む30分以上）で再測定し、移動中もgapが7.4〜7.6秒に張り付くか確認する。歩行中に5秒±2秒（3〜7秒）に収まるようであれば、課題1は「静止時の省電力挙動」として扱い、以下2〜4の設定変更は不要と判断できる。

2. **`LocationRequest`設定の見直し**（上記1で歩行中も改善しない場合）
...
6. **判定基準の見直し検討（最終手段）**
   上記1〜5で改善しない場合、実測値ベースで許容範囲（現行3〜7秒）の妥当性を[background-tracking.md](../../02-architecture-design/background-tracking.md)側の担当者と協議し、取得間隔設定または判定基準そのものの見直しを検討する。
