...
- 取得間隔 10秒、精度モード `BALANCED_POWER_ACCURACY`
...
|---|---------|----------------|----------------------|
| 1 | GPS取得の安定性 | 10秒間隔でGPS取得が欠落・遅延なく継続するか | background-tracking.md「GPS取得」 |
...
| GPS取得 | `FusedLocationProviderClient`、10秒間隔、`PRIORITY_BALANCED_POWER_ACCURACY` |
| バックグラウンド動作 | `ForegroundService`（常駐通知を表示） |
| 権限要求 | `ACCESS_FINE_LOCATION` / `ACTIVITY_RECOGNITION` / `POST_NOTIFICATIONS` を起動時にランタイム要求 |
| データ保存 | Room等の永続化は行わず、取得点を **Logcat出力** ＋ 画面上のリスト表示に加え、記録開始ごとにアプリ専用外部ストレージ領域へ **CSVファイル出力**（`Android/data/<パッケージ名>/files/gps_poc_log_<開始日時>.csv`）。PC・USB接続なしで屋外検証できるようにするための出力で、記録項目はLogcatと同一（件数・タイムスタンプ・緯度経度・精度(accuracy)・取得間隔・遅延有無・バッテリー残量） |
...
4. Logcat出力（PC接続時）または端末に出力されたCSVログファイル（PC接続なしの屋外検証時、検証終了後に`adb pull`で回収）と画面表示から、検証項目1・2・3・5の結果を記録する
5. 開始時・終了時のバッテリー残量から消費率を算出し、12時間換算で検証項目4を評価する（バッテリー消費測定中はUSB給電による充電の影響を避けるため、PC接続を伴うLogcatのリアルタイム監視は行わずCSVログファイルを使用する）
...
| 検証項目 | 合格ライン |
|---------|-----------|
| 1. GPS取得の安定性 | 取得間隔が10秒±2秒以内に収まる割合が95%以上 |
...
| 6. 権限検知 | 権限剥奪から次回GPS取得ループ（最大10秒後）以内に検知できる |
